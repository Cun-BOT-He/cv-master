...
class RCNN_CosSim(M.Module):
    ...
    def __init__(self, cfg):
        ...
        # box predictor
        self.scale = cfg.rcnn_cosine_scale
        if self.scale == -1:
            # learnable global scaling factor
            self.scale = megengine.Parameter(F.ones(1) * 20.0)
    ...
    def forward(self, fpn_fms, rcnn_rois, im_info=None, gt_boxes=None):
        ...
        # normalize input roi_feature
        feature_norm = F.norm(roi_feature, ord = 2, axis = 1, keepdims = True)
        feature_normalized = roi_feature.div(feature_norm + 1e-5)

        # normalize weight
        weight_norm = (
            F.norm(self.pred_cls.weight.data, ord = 2, axis = 1, keepdims = True)
        )
        self.pred_cls.weight.data = self.pred_cls.weight.data.div( weight_norm + 1e-5 )

        cos_dist = self.pred_cls(feature_normalized)
        pred_logits = self.scale * cos_dist
